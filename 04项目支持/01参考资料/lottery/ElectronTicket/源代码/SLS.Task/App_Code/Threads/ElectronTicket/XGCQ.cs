...
namespace SLS.Task
{
    ...
    /// XGCQ 的摘要说明
    /// </summary>
    public class ElectronTicket_XGCQ
    {
        ...
        private Message msg = new Message("ElectronTicket_XGCQ");
        private Log log = new Log("ElectronTicket_XGCQ");

        public int State = 0;   // 0 停止 1 运行中 2 置为停止

        public ElectronTicket_XGCQ(string connectionstring)
        {
        ...
        public void Do()
        {
            while (true)
            {
                if (State == 2)
                {
                    msg.Send("ElectronTicket_XGCQ Stop.");
                    log.Write("ElectronTicket_XGCQ Stop.");
