...
function init() {
    ...
    o_lab_SumMoney.innerHTML = "0.00";
    o_lab_ShareMoney.innerHTML = "0.00";
    o_lab_AssureMoney.innerHTML = "0.00";
    o_lab_BuyMoney.innerHTML = "0.00";
...
function GetSchemeBonusScalec() {
    if (!isGetSchemeBonusScalec) {
        try {
            Lottery_Buy_3D.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);
        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_3D.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        Lottery_Buy_3D.GetSysTime(GetServerTime_callback);
    }
    catch (e) {
        //如果失败了，就继续读取
...
function GetIsuseInfo(lotteryID) {
    currentLotteryID = lotteryID;
    try {
        Lottery_Buy_3D.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
    }
    catch (e) {
...
function GetIsuseInfo_callback(response) {
    ...
    document.getElementById('lastIsuseInfo').innerHTML = lastIsuse;
    miss = arrInfo[3];
            ...
            if (Miss_s.length == 3) {
                for (var i = 0; i < 3; i++) {
                    ...
                    if (Miss.length != 10)
                        continue;
                    for (var j = 0; j < 10; j++) {
                        var o_Miss = document.iframe_playtypes.document.getElementById("Miss_" + (i.toString()) + "_" + j.toString());
    ...
    $Id("currIsuseEndTime").innerHTML = arrcurrIsuse[2].replace("/", "-").replace("/", "-");
    $Id("HidIsuseEndTime").value = arrcurrIsuse[2];
}

//获取专家信息(当彩种为福彩时)
var time_GetBindFCExpertList = null;
function GetBindFCExpertList(lotteryID) {
    currentLotteryID = lotteryID;
    try {
        Lottery_Buy_3D.GetFCExpertList(lotteryID, GetBindFCExpertList_callback);
    }
    catch (e) {
        time_GetBindFCExpertList = setTimeout("GetBindFCExpertList(" + lotteryID + ");", 2000);
    }
}

function GetBindFCExpertList_callback(response) {
    if (response == null || response.value == null) {
        time_GetBindFCExpertList = setTimeout("GetBindFCExpertList(" + currentLotteryID + ");", 2000);
        return;
    }

    //将time_GetBindFCExpertList移除
    if (time_GetBindFCExpertList != null) {
        clearTimeout(time_GetBindFCExpertList);
    }

    FCExperts_lcal = response.value;
    BindFCExpertList(0);
}

function BindFCExpertList(type) {
    if (FCExperts_lcal == null) {
        time_GetBindFCExpertList = setTimeout("GetBindFCExpertList(" + currentLotteryID + ");", 2000);
        return;
    }

    var experts = FCExperts_lcal.split('|');
...
function GetBindWinNumber(lotteryID) {
    currentLotteryID = lotteryID;
    try {
        Lottery_Buy_3D.GetWinNumber(lotteryID, GetBindWinNumber_callback);
    }
    catch (e) {
...
function BindWinNumber(type) {
    ...
    headHtm += "</table>";
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/FC3D/FC3D_ZHXT.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function loadLottery(lotteryID) {
    ...
    //获取专家列表信息
    if ($Id("ExpertList") != undefined && $Id("ExpertList") != null) {
        GetBindFCExpertList(lotteryID);
    }
}
//---------------------------------------投注功能区代码-------------------------------------------------
...
function PlayTypeIntroduce(lotteryID) {
    ...
    $Id("divSchemeAll").style.display = "none";
    $Id("iframePlayTypeIntroduce").src = "../Home/Room/Help/help_6.htm";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_3D.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function BetLuckNumber(lotteryID) {
    ...
    o_lab_Num.innerHTML = StrToInt(o_lab_Num.innerHTML) + 1;
    ResetShare();
    CalcResult();
    calculateAllMoney();
...
function showSameHeight() {
    /*if (document.getElementById("menu_left").clientHeight < document.getElementById("menu_right").clientHeight) {
        ...
        if (document.getElementById("menu_right").offsetHeight >= 860) {
            document.getElementById("menu_left").style.height = (document.getElementById("menu_right").offsetHeight + 50) + "px";
        }
        else {
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_3D.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {document.getElementById('tbPlayTypeID').value = t;
    ...
    switch (t) {
        case '602':
            playTypeName = '直选复式';
            strhtml='../Lottery/playtypes/fc3d/FC3D_ZhiF.htm';
			$Id("btn_2_Rand").disabled = true;
            $Id("btn_2_Rand5").disabled = true;
            break;
        case '603':
            playTypeName = '组选单式';
            strhtml='../Lottery/playtypes/fc3d/FC3D_ZuD.htm';
			$Id("btn_2_Rand").disabled = true;
            $Id("btn_2_Rand5").disabled = true;
            break;
        case '604':
            playTypeName = '组选6复式';
            strhtml='../Lottery/playtypes/fc3d/FC3D_Zu6F.htm';
            break;
        case '605':
            playTypeName = '组选3复式';
            strhtml='../Lottery/playtypes/fc3d/FC3D_Zu3F.htm';
            break;
        case '606':
            playTypeName = '直选包点';
            strhtml='../Lottery/playtypes/fc3d/FC3D_ZhiB.htm';
			$Id("btn_2_Rand").disabled = true;
            $Id("btn_2_Rand5").disabled = true;
            break;
        case '607':
            playTypeName = '组选包点';
            strhtml='../Lottery/playtypes/fc3d/FC3D_ZuB.htm';
            break;
        default:
            t='601';
            playTypeName = '直选单式';
            strhtml='../Lottery/playtypes/fc3d/FC3D_ZhiD.htm';
            break;
    }
