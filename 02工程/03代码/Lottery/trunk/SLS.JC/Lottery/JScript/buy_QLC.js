...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_QLC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_QLC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QLC.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QLC.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetIsuseInfo_callback(response) {
    ...
    document.getElementById('currIsuseName').innerHTML = arrcurrIsuse[1];
    document.getElementById('currIsuseEndTime').innerHTML = arrcurrIsuse[2].replace("/", "-").replace("/", "-");
...
function GetBindFCExpertList(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QLC.GetFCExpertList(lotteryID, GetBindFCExpertList_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QLC.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/QLC/7LC_CGXMB.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
   ...
   var arrTable = $Id("div_QH_Today").getElementsByTagName("table")[0].getElementsByTagName("tr");
    for (j = 0; j < arrTable.length; j++) {
		...
		if (objs.checked == true) {
			...
			//obj_txtMoney.innerHTML = obj_txtMoney.value;
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_QLC.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_QLC.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {document.getElementById('tbPlayTypeID').value = t;
    ...
    var hosthtml = gethosturl() + '/Lottery/'; var strhtml = "";
        ...
        case '1302':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/QLC/QLC_F.htm';
            break;

        default:
            t = '1301';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/QLC/QLC_D.htm';
            break;
        }
