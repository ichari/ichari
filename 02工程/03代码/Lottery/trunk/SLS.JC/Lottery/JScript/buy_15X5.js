...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_15X5.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_15X5.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_15X5.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_15X5.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetBindFCExpertList(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_15X5.GetFCExpertList(lotteryID, GetBindFCExpertList_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_15X5.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/HD15X5/C15X5_CGXMB.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_15X5.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_15X5.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {document.getElementById('tbPlayTypeID').value = t;
    ...
    var hosthtml = gethosturl() + '/Lottery/'; var strhtml = "";
        ...
        case '5902':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/AHFC15X5/AHFC15X5_F.htm';
            break;

        case '5903':
            playTypeName = '胆拖';
            strhtml = '../Lottery/playtypes/AHFC15X5/AHFC15X5_DT.htm';
            break;

        default:
            t = '5901';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/AHFC15X5/AHFC15X5_D.htm';
            break;
        }
...
function ResetShare() {
    ...
    if ($Id("CoBuy").checked) {
        ...
        o_tb_BuyShare.value = Math.round(SumMoney * Opt_InitiateSchemeLimitScale) < 1 ? 1 : Math.round(SumMoney * Opt_InitiateSchemeLimitScale);
    }
}
