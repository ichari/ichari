...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_SFC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

            isGetSchemeBonusScalec = true;
        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_SFC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_SFC.GetSysTime(GetServerTime_callback);
...
function GetAddone(lotteryID, IssueId) {
    ...
    try {
        ...
        Lottery_Buy_SFC.GetAddone(lotteryID, IssueId, GetAddone_callback);

    }
    catch (e) {

        time_GetAddone = setTimeout("Lottery_Buy_SFC.GetAddone(" + lotteryID + "," + IssueId + " ,GetAddone_callback);", 2000);
    }
...
function GetAddone_callback(response) {
    ...
    if (response == null || response.value == null) {
        ...
        time_GetAddone = setTimeout("Lottery_Buy_SFC.GetAddone(" + currentLotteryID + "," + $Id("HidIsuseID").value + ",GetAddone_callback);", 2000);
...
function ClickHMTJ(obj) {
    ...
    if (lastHMTJ != null) {
        ...
        $Id(lastHMTJ.id.replace("href", "tb")).style.display = "none";
    }
    ...
    $Id(obj.id.replace("href", "tb")).style.display = "";
    lastHMTJ = obj;
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_SFC.GetIsuseInfo(lotteryID, $Id('ddlIsuses').value, GetIsuseInfo_callback);
...
function GetIsuseInfo_callback(response) {
    ...
    currIsuseName.innerText = arrcurrIsuse[2].replace("/", "-").replace("/", "-");
...
function GetNewsInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_SFC.GetNewsInfo(lotteryID, GetNewsInfo_callback);
...
function GetBindZCExpertList(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_SFC.GetZCExpertList(lotteryID, GetBindZCExpertList_callback);
...
function checkSchemeMoney(obj) {
    ...
    if (($Id("Radio2").checked) && obj.id == "tb_MaxSchemeMoney") {
        ...
        if (parseInt(obj.value) > parseInt($Id("tb_MinSchemeMoney").value) * 1.4) {
            obj.value = parseInt(parseInt($Id("tb_MinSchemeMoney").value) * 1.4);
        }
...
function UpdateLotteryNumber() {
    ...
    for (i = 0; i < o_list_LotteryNumber.length; i++) {
        if (o_list_LotteryNumber.options[i].selected)
            $Id("HidSelectedLotteryNumber").value = o_list_LotteryNumber.options[i].text.trim();
    }
...
function CreateUplaodDialog() {
    ...
    var o_tbPlayTypeID = $Id("tbPlayTypeID");
    ...
    var dialog = '<table><tr><td style="background-color: #AFBCD6; padding: 10px;font-size:12px"><table style="width: 100%;background-color:White;" border="0" cellpadding="0" cellspacing="1"><tr><td height="36" bgcolor="#6D84B4" class="bai14" style="padding: 0px 10px 0px 15px;text-align:left;"><span id="lbLotteryName"></span> 第 <span id="lbIsuse"></span>&nbsp;期 粘贴投注</td></tr><tr><td style="padding: 5px;" align="center"><textarea id="tbLotteryNumbers" style="width:98%; height:160px;"></textarea></td></tr><tr><td><table width="100%" border="0" align="right" cellpadding="0" cellspacing="0"><tr><td style="text-align:left;"><table cellpadding="0" cellspacing="0" style="width:100%;"><tr><td style="text-align:right;">方案上传：</td><td colspan="2"><iframe id="frame_Upload" name="frame_Upload" frameborder="0" src="../Home/Room/SchemeUpload.aspx?id=' + $Id('HidLotteryID').value + '&amp;PlayType=' + o_tbPlayTypeID.value + '&amp;Isuse=' + $Id('HidIsuseID').value + '" width="100%" scrolling="no" height="30"></iframe></td></tr></table></td></tr><tr><td style="text-align:right; padding-right:10px;"><font color="#ff0000">【注】</font>如果选择方案文件<font color="#ff0000">(.txt格式)</font>上传,上面的投注内容将被清除并被替换成方案文件里面的内容。<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 方案文件中请输入规范的投注内容，多注请用回车换行。 <span class="blue12"><a href="../Home/Room/SchemeExemple.aspx?id=' + $Id("HidLotteryID").value + '" target="_blank">请参看格式规范</a></span></td></tr><tr><td style="background-color:#f2f2f2; padding:10px;"><table width="280" border="0" align="right" cellpadding="0" cellspacing="0"><tbody style="cursor: pointer; color: White;"><tr><td width="19%" align="right"><table width="88" border="0" cellpadding="0" cellspacing="1" bgcolor="#FF3300"><tr><td height="23" align="center" bgcolor="#FD9A00" onclick=" btn_OK();">确定</td></tr></table></td><td width="32%" align="right"><table width="88" border="0" cellpadding="0" cellspacing="1" bgcolor="#FF3300"><tr><td height="23" align="center" bgcolor="#FD9A00" onclick=" btn_Close();">取消</td></tr></table></td><td width="32%" align="right"><table width="88" border="0" cellpadding="0" cellspacing="1" bgcolor="#FF3300"><tr><td height="23" align="center" bgcolor="#FD9A00" onclick=" btn_Close();">关闭窗口</td></tr></table></td></tr></tbody></table></td></tr></table></td></tr></table></td></tr></table>';

    txt.innerHTML = dialog;

    $Id("msgDiv2").appendChild(txt);
    $Id("tbLotteryNumbers").focus();

    $Id("lbIsuse").innerHTML = $Id('ddlIsuses').value;
    $Id("lbLotteryName").innerHTML = LotteryName;

    $Id("list_LotteryNumber").style.display = "none";
}
...
function btn_Close() {
    ...
    try {
        $Id("list_LotteryNumber").style.display = "";
    } catch (e) { }
    $Id("list_LotteryNumber").style.display = "";
}
...
function showSameHeight() {
    if ($Id("menu_left").clientHeight < $Id("menu_right").clientHeight) {
        $Id("menu_left").style.height = $Id("menu_right").offsetHeight + "px";
    }
    else {
        if ($Id("menu_right").offsetHeight >= 860) {
            $Id("menu_left").style.height = ($Id("menu_right").offsetHeight + 250) + "px";
        }
        else {
            $Id("menu_left").style.height = "860px";
        }
    }
}
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_SFC.AnalyseScheme($Id("tbLotteryNumbers").value, $Id('HidLotteryID').value, $Id('tbPlayTypeID').value);
...
function btn_OKClick() {
    ...
    if ($Id("currIsuseName").innerHTML == "本期已截止投注" < 0) {
        msg("本期投注已截止，谢谢。");
...
function PageEvent() {
 ...
 function clickPlayType(t) {
        ...
        case '7402':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/SFC/SFC_F.htm';
            break;
         ...
         case '74011':
            ...
            playTypeID = '7401';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/SFC/SFC_D.htm';
            break;
        ...
        iframe_playtypes.location.href = (strhtml).replace(new RegExp("../Lottery/"), hosthtml);
            ...
            $Id('tbPlayTypeID').value = playTypeID;
            resetPage();
...
function ResetShare() {
    ...
    if ($Id("CoBuy").checked) {
        ...
        o_tb_BuyShare.value = Math.round(SumMoney / 2 * Opt_InitiateSchemeLimitScale) < 1 ? 1 : Math.round(SumMoney / 2 * Opt_InitiateSchemeLimitScale);
    }
}