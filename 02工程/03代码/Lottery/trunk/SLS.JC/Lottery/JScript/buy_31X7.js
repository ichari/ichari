...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_31X7.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_31X7.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_31X7.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_31X7.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_31X7.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_31X7.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        alert(v);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_31X7.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {document.getElementById('tbPlayTypeID').value = t;
    ...
    var hosthtml = gethosturl() + '/Lottery/'; var strhtml = "";
        ...
        case '6502':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/TC31X7/TC31X7_F.htm';
            break;

        default:
            t = '6501';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/TC31X7/TC31X7_D.htm';
            break;
    }
