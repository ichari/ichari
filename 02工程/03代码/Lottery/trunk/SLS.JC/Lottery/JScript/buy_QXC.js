...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_QXC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_QXC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QXC.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QXC.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_QXC.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/7Xing/Default.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_QXC.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function showSameHeight() {
    if (document.getElementById("menu_left").clientHeight < document.getElementById("menu_right").clientHeight) {
        ...
        if (document.getElementById("menu_right").offsetHeight >= 86) {
            document.getElementById("menu_left").style.height = document.getElementById("menu_right").offsetHeight + "px";
        }
        else {
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_QXC.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function PageEvent(lotteryID) {
 ...
 function clickPlayType(t) {document.getElementById('tbPlayTypeID').value = t;
     ...
     var hosthtml = gethosturl() + '/Lottery/'; var strhtml = "";
        ...
        case '302':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/QXC/QXC_F.htm';
            break;

        default:
            t = '301';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/QXC/QXC_D.htm';
            break;
        }
