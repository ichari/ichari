...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_PL5.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_PL5.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL5.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL5.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetNewsInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL5.GetNewsInfo(lotteryID, GetNewsInfo_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL5.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/PL5/Default.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_PL5.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_PL5.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {
    ...
    switch (t) {
        ...
        case '6402':
            playTypeName = '排列5复式';
            strhtml = '../Lottery/playtypes/SZPL5/SZPL5_F.htm';
            break;

        default:
            t = '6401';
            playTypeName = '排列5单式';
            strhtml = '../Lottery/playtypes/SZPL5/SZPL5_D.htm';
            break;
    }
