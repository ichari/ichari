...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_JQC.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

            isGetSchemeBonusScalec = true;
        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_JQC.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_JQC.GetSysTime(GetServerTime_callback);
...
function GetAddone(lotteryID, IssueId) {
    ...
    try {
        ...
        Lottery_Buy_JQC.GetAddone(lotteryID, IssueId, GetAddone_callback);

    }
    catch (e) {

        time_GetAddone = setTimeout("Lottery_Buy_JQC.GetAddone(" + lotteryID + "," + IssueId + " ,GetAddone_callback);", 2000);
    }
...
function GetAddone_callback(response) {
    ...
    if (response == null || response.value == null) {
        ...
        time_GetAddone = setTimeout("Lottery_Buy_JQC.GetAddone(" + currentLotteryID + "," + $Id("HidIsuseID").value + ",GetAddone_callback);", 2000);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_JQC.GetIsuseInfo(lotteryID, $Id('ddlIsuses').value, GetIsuseInfo_callback);
...
function GetNewsInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_JQC.GetNewsInfo(lotteryID, GetNewsInfo_callback);
...
function GetBindZCExpertList(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_JQC.GetZCExpertList(lotteryID, GetBindZCExpertList_callback);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_JQC.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {
    ...
    switch (t) {
        case '202':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/JQC/JQC_F.htm';
            break;
        ...
        case '2011':
            ...
            playTypeID = '201';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/JQC/JQC_D.htm';
            break;
