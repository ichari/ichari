...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_DF6J1.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_DF6J1.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_DF6J1.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_DF6J1.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetBindFCExpertList(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_DF6J1.GetFCExpertList(lotteryID, GetBindFCExpertList_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_DF6J1.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/DF6J1/DF61_ZHFB.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_DF6J1.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_DF6J1.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {document.getElementById('tbPlayTypeID').value = t;
    ...
    var hosthtml = gethosturl() + '/Lottery/'; var strhtml = "";
        ...
        case '5802':
            playTypeName = '复式';
            strhtml = '../Lottery/playtypes/DF6J1/DF6J1_F.htm';
            break;

        default:
            t = '5801';
            playTypeName = '单式';
            strhtml = '../Lottery/playtypes/DF6J1/DF6J1_D.htm';
            break;
        }
