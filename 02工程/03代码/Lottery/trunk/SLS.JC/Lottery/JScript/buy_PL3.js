...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Lottery_Buy_PL3.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Lottery_Buy_PL3.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL3.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL3.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetIsuseInfo_callback(response) {
    ...
    if (currentLotteryID == 63) {
        $Id("SZPL3Zu3MissCount").innerHTML = $Id("lbMiss").innerHTML.split('|')[1];
    }
...
function GetNewsInfo(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL3.GetNewsInfo(lotteryID, GetNewsInfo_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Lottery_Buy_PL3.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"../TrendCharts/PL3/Default.aspx\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Lottery_Buy_PL3.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        msg(v);
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Lottery_Buy_PL3.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function clickPlayType(t) {
    ...
    switch (t) {
        case '6302':
            ...
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_ZhiF.htm';
            break;

        case '6303':
            playTypeName = '排列3组选单式';
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_ZuD.htm';
            break;

        case '6304':
            playTypeName = '排列3组选6复式';
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_Zu6F.htm';
            break;

        case '6305':
            playTypeName = '排列3组选3复式';
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_Zu3F.htm';
            break;

        case '6306':
            playTypeName = '排列3直选和值';
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_ZhiH.htm';
            break;

        case '6307':
            playTypeName = '排列3组选和值';
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_ZuH.htm';
            break;

        default:
            t = '6301';
            playTypeName = '直选单式';
            strhtml = '../Lottery/playtypes/SZPL3/SZPL3_ZhiD.htm';
            break;
    }
