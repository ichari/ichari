...
function init() {
    ...
    o_tb_Title.value = "";
    o_lab_Num.innerHTML = "0";
    o_lab_SumMoney.innerHTML = "0";
    o_lab_ShareMoney.innerHTML = "0";
    o_lab_AssureMoney.innerHTML = "0";
    o_lab_BuyMoney.innerHTML = "0";
    o_tb_Price = 2;
...
function GetIsuseInfo_callback(response) {
    ...
    if ($Id("div_QH_Today").childNodes[0].rows.length > 1) {
        $Id("div_QH_Today").innerHTML = chaseIsuse;
        try {
        ...
        if ($Id("HidIsuseID").value != arrcurrIsuse[0]) {
            $Id("div_QH_Today").childNodes[0].deleteRow(0);
            accountAllMoney();
        }
    }
	//alert(arrcurrIsuse[1])
    $Id("currIsuseName").innerHTML = arrcurrIsuse[1];
...
function btn_ClearClick() {
    try {
        ...
        ResetShare();
        CalcResult();
       /* if (document.getElementById("tbPlayTypeName").value = '大小单双	') {
            document.getElementById("btn_2_Rand").disabled = true;
            document.getElementById("btn_2_Rand5").disabled = true;
        }
        else {
            document.getElementById("btn_2_Rand").disabled = false;
            document.getElementById("btn_2_Rand5").disabled = false;
...
function btn_ClearSelectClick() {
    ...
    for (i = o_list_LotteryNumber.length - 1; i >= 0; i--) {
        if (o_list_LotteryNumber.options[i].selected) {
            ...
            o_lab_Num.innerHTML = StrToInt(o_lab_Num.innerHTML) - Num;
...
function CheckShare() {
    ...
    if (Share < 0) {
        ...
        if (Share > 1) {
            var multiple = StrToInt(o_tb_Multiple.value);
            var SumNum = StrToInt(o_lab_Num.innerHTML);
...
function CalcResult() {
    ...
    o_lab_SumMoney.innerHTML = SumMoney;
    o_lab_ShareMoney.innerHTML = ShareMoney;
    o_lab_AssureMoney.innerHTML = AssureMoney;
    o_lab_BuyMoney.innerHTML = BuyMoney;
    if ($Id("Chase").checked)
        accountAllMoney();
}
...
function accountAllMoney() {
    ...
    var arrTable = $Id("div_QH_Today").getElementsByTagName("table")[0].getElementsByTagName("tr");
    for (var j = 0; j < arrTable.length; j++) {
        var objs = arrTable[j].getElementsByTagName("input")[0];
        var obj_txtMoney, obj_txtBNum,lab_SumMoney = $Id("lab_SumMoney");
			if (objs.checked == true) {				
				obj_txtBNum = $Id(objs.id.replace("check", "times"));
				obj_txtMoney = $Id(objs.id.replace("check", "money"));
				obj_Name = $Id(objs.id.substring(0, (objs.id.length - 6)));
				obj_txtMoney.value = parseInt(obj_txtBNum.value) * parseInt(lab_SumMoney.innerHTML);
				obj_txtMoney.value = obj_txtMoney.value;
				needMoney = parseInt(needMoney) + parseInt(obj_txtMoney.value);
				isuseCount++;
				lastNum = parseInt(obj_txtBNum.value);
			}
	}
    $Id("LbSumMoney").innerHTML = needMoney;

    if (needMoney < 1) {
        $Id("spanChaseIsuseCount").innerHTML = "0";
        $Id("spanWinMoney").innerHTML = "0";
        $Id("spanSchemeProfit").innerHTML = "0";
        $Id("spanFJL").innerHTML = "0";
    } else {
        $Id("spanChaseIsuseCount").innerHTML = isuseCount;
        $Id("spanWinMoney").innerHTML = lastNum * parseFloat(document.getElementById("spanPlayTypeMoney").innerHTML);
        $Id("spanSchemeProfit").innerHTML = parseFloat($Id("spanWinMoney").innerHTML) - needMoney;
        $Id("spanFJL").innerHTML = Round(parseFloat($Id("spanWinMoney").innerHTML) / needMoney, 2);
    }
}
...
function resetPage() {
    $Id("Chase").checked = false;
    $Id("CoBuy").checked = false;
    $Id("trShowJion").style.display = "none"; //合买方案
    $Id("trGoon").style.display = "none"; //我要追号
...
function Cb_CheckAll() {
    var o_cb_All = $Id("cb_All").checked;
    var table = $Id("div_QH_Today").getElementsByTagName("table")[0];
    var rows = table.getElementsByTagName("tr");
    var num = rows.length;
    for (var i = 1; i < num; i++) {
        var obj = rows[i].getElementsByTagName("td")[0].getElementsByTagName("input")[0];
        obj.checked = o_cb_All?true:false;
		check(obj);
    }
}
...
function ChangeBackImage(index) {
    ...
    var tab = table.getElementsByTagName("div");
    var arr = new Array(1, 3, 5, 7, 9);
    for (var i = 0; i < arr.length; i++) {
        if (index == arr[i]) {
            tab[arr[i]].className = "redMenu";
        }
        else {
            tab[arr[i]].className = "whiteMenu";
        }
    }
}
...
function playTypeIntroduce(lotteryID) {
    ...
    if ($Id("iframePlayTypeIntroduce").src == "") {
        $Id("iframePlayTypeIntroduce").src = "../Home/Room/Help/help_61.htm";
    }
    ChangeBackImage(7);
}
...
function clickPlayType(t) {
    var playTypeName = '';
    var hosthtml = gethosturl() + '/Lottery/';
    var strhtml = "";
    switch (t) {
        case 'playType2801':
            playTypeName = '单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码万、千、百、十、个位与开奖号码全部相同且顺序一致，奖金 <span style="color:red" id="spanPlayTypeMoney">100000</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_5XDS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2802':
            playTypeName = '复选';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;包括五星、三星、二星、一星投注号码各一注，最高奖金可达<span style="color:red" id="spanPlayTypeMoney">100000</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_5XFS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2803':
            playTypeName = '组合';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码万、千、百、十、个位与开奖号码全部相同且顺序一致，奖金<span style="color:red" id="spanPlayTypeMoney">100000</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_5XZH.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2804':
            playTypeName = '五星通选单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;一注号码，三个奖级通吃，五次中奖机会，大奖<span style="color:red" id="spanPlayTypeMoney">20000</span> 元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_5XTXDS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2805':
            playTypeName = '五星通选复式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;一注号码，三个奖级通吃，五次中奖机会，大奖 <span style="color:red" id="spanPlayTypeMoney">20000</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_5XTXFS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2806':
            playTypeName = '单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码百、十、个位与开奖号码全部相同且顺序一致，奖金 <span style="color:red" id="spanPlayTypeMoney">1000</span>元。  ';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_3XDS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2807':
            playTypeName = '复选';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;包括三星、二星、一星投注号码各一注，最高奖金可达 <span style="color:red" id="spanPlayTypeMoney">1000</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_3XFS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2808':
            playTypeName = '组合';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码百、十、个位与开奖号码全部相同且顺序一致，奖金<span style="color:red" id="spanPlayTypeMoney">1000</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_3XZH.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2809':
            playTypeName = '三星组三单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;选择2个数字(其中有一个数字必须选择两次)进行组三投注。奖金<span style="color:red" id="spanPlayTypeMoney">320</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2_3.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2810':
            playTypeName = '三星组三复式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;从0-9共10个数中任选2-10个号码进行组三的投注。奖金<span style="color:red" id="spanPlayTypeMoney">320</span>元。'
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2_4.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2811':
            playTypeName = '三星组六单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;选择3个数字(不能出现重复)进行组六投注。奖金<span style="color:red" id="spanPlayTypeMoney">160</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2_5.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2812':
            playTypeName = '三星组六复式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;&nbsp;从0-9共10个数中任选4-10个号码进行组六的投注。奖金<span style="color:red" id="spanPlayTypeMoney">160</span>元。'
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2_6.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2813':
            playTypeName = '单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码十、个位与开奖号码全部相同且顺序一致，奖金<span style="color:red" id="spanPlayTypeMoney">100</span>元。  ';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XDS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2814':
            playTypeName = '复选';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;包括二星、一星投注号码各一注，最高奖金可达<span style="color:red" id="spanPlayTypeMoney">100</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XFS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2815':
            playTypeName = '组合';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;选择十、个位两个号码进行投注，奖金<span style="color:red" id="spanPlayTypeMoney">100</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XZH.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2816':
            playTypeName = '二星组选单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;选择两个数字进行投注，不包含对子，奖金<span style="color:red" id="spanPlayTypeMoney">50</span>元。 '
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XZXDS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2817':
            playTypeName = '二星组选复式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;任选3至7个不同数字进行投注，开奖号码不排序，奖金<span style="color:red" id="spanPlayTypeMoney">50</span>元，开奖号码为对子奖金<span style="color:red">  100</span>元。';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XZXFS.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        case 'playType2818':
            playTypeName = '二星组选包点';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;十、个位两个号码之和，开奖号码不排序，奖金<span style="color:red" id="spanPlayTypeMoney">50</span>元，开奖号码为对子奖金<span style="color:red">  100</span>元。 '
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XZXBD.htm';
			$Id("btn_2_Rand").disabled = "false";
			$Id("btn_2_Rand5").disabled = "false";
            break;
        ...
        case 'playType2819':
            playTypeName = '二星组选包胆';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;选择一个数字进行投注，奖金<span style="color:red" id="spanPlayTypeMoney">50</span>元，开奖号码为对子奖金<span style="color:red"> 100</span>元。  ';
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_2XZXBDan.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2820':
            playTypeName = '单式';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码个位与开奖号码全部相同且顺序一致，奖金<span style="color:red" id="spanPlayTypeMoney">10</span>元。  '
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_1XDS.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2821':
            playTypeName = '组合';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;投注号码个位与开奖号码全部相同且顺序一致，奖金<span style="color:red" id="spanPlayTypeMoney">10</span>元。;'
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_1XZH.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        case 'playType2822':
            playTypeName = '大小单双';
            $Id("labShowWinMoney").innerHTML = '&nbsp;&nbsp;分别从个、十位中任选一种性质组成一注投注号码，奖金<span style="color:red" id="spanPlayTypeMoney">4</span>元。 '
            strhtml = '../Lottery/playtypes/CQSSC/CQSSC_CDX.htm';
			$Id("btn_2_Rand").disabled = "";
			$Id("btn_2_Rand5").disabled = "";
            break;
        default:
...
function clickPlayClass(i, obj, pltype) {
    ...
    $Id(pltype).checked = true;
    clickPlayType(pltype);
}
...
function btn_OK() {
    ...
    //document.getElementById("tbLotteryNumbers").value = "";
    ResetShare();
    CalcResult();
...
//    if (o_list_LotteryNumber.length > 5) {
//        msg("每次最多投注5注");
//        return false;
//    }
    var multiple = StrToInt(o_tb_Multiple.value);
    ...
    var BuyShare = StrToInt(o_tb_BuyShare.value);
    var AssureShare = StrToInt(o_tb_AssureShare.value);
    var SumMoney = StrToInt(o_lab_SumMoney.innerHTML);
    var AssureMoney = StrToFloat(o_lab_AssureMoney.innerHTML);
    var BuyMoney = StrToFloat(o_lab_BuyMoney.innerHTML);
    if (SumNum < 1) {
        msg("请输入投注内容。");
        return false;
    }
    ...
    if (StrToFloat(o_lab_ShareMoney.innerHTML) < 1) {
        msg("每份金额不能小于 1 元。");
        o_tb_Share.focus();
        return false;
    }
    ...
    //追号
    if ($Id("Chase").checked) {
        if (StrToInt($Id("LbSumMoney").innerHTML) > 0) {
            ...
            $Id("tb_hide_ChaseBuyedMoney").value = $Id("LbSumMoney").innerHTML;

            var TipStr = '您要申请' + LotteryName + $Id("tbPlayTypeName").value + '投注，详细内容：\n\n';

            TipStr += "　　总金额：　" + $Id("LbSumMoney").innerHTML + " 元\n\n";
        ...
        if ((BuyShare) < Math.round(Share * Opt_InitiateSchemeLimitScale)) {
            if (Opt_InitiateSchemeLimitLowerScale == Opt_InitiateSchemeLimitUpperScale) {
                msg("发起人最少必须认购 " + (Opt_InitiateSchemeLimitLowerScale * 100) + "%。(" + Math.round(Share * Opt_InitiateSchemeLimitLowerScale) + ' 份， ' + (Math.round(Share * Opt_InitiateSchemeLimitLowerScale) * StrToFloat(o_lab_ShareMoney.innerHTML)) + ' 元)');
            }
            else {
        ...
        TipStr += "　　注　数：　" + SumNum + "\n";
        TipStr += "　　倍　数：　" + multiple + "\n";
        TipStr += "　　总金额：　" + o_lab_SumMoney.innerHTML + " 元\n\n";
        TipStr += "　　总份数：　" + Share + " 份\n";
        TipStr += "　　每　份：　" + o_lab_ShareMoney.innerHTML + " 元\n\n";
        TipStr += "　　保　底：　" + AssureShare + " 份，" + o_lab_AssureMoney.innerHTML + " 元\n";
        TipStr += "　　购　买：　" + BuyShare + " 份，" + o_lab_BuyMoney.innerHTML + " 元\n\n";

        if (!confirm(TipStr + "按“确定”即表示您已阅读《" + LotteryName + "投注协议》并立即提交方案，确定要提交方案吗？"))
            return false;
    }
    $Id("tb_hide_SumMoney").value = o_lab_SumMoney.innerHTML;
    $Id("tb_hide_AssureMoney").value = o_lab_AssureMoney.innerHTML;
    $Id("tb_hide_SumNum").value = o_lab_Num.innerHTML;
    __doPostBack('btn_OK', '');
}
...
function CheckShare2(type, obj) {
    ...
    if (!OK) {
        ...
        if ((buyedShare) < Math.round(share * Opt_InitiateSchemeLimitScale)) {
            if (Opt_InitiateSchemeLimitLowerScale == Opt_InitiateSchemeLimitUpperScale) {
                msg("发起人最少必须认购 " + (Opt_InitiateSchemeLimitLowerScale * 100) + "%。(" + Math.round(share * Opt_InitiateSchemeLimitLowerScale) + ' 份， ' + (Math.round(share * Opt_InitiateSchemeLimitLowerScale) * money / share) + ' 元)');
            }
            else {
                msg("此方案发起人认购(不含保底)最少必须达到 " + Math.round(share * Opt_InitiateSchemeLimitScale) + " 份。\r\n\r\n" +
                    "发起方案最少认购比例说明：\r\n" +
