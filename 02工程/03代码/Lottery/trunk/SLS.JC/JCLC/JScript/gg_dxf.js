...
selectTable.getNewTr = function(tr, chk, node) {//添加行
    ...
    if (!node) {
        ...
        //tmp.pn = parseFloat(attr(tr, ["", "big", "small"][tmp.spf])) || 0;
...
selectTable.isAllCancel = function(tr) {
    var test = true;
    each(tr.getElementsByTagName('INPUT'), function() { if (this.parentNode.style.display != 'none') return test = false }, 1, 3);
    each(tr.getElementsByTagName('INPUT'), function() { this.checked = false; }, 3, 4);
    return test
};
