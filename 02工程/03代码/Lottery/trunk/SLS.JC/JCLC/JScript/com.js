...
var selectTable = {
    ...
    __getPn__: function(tr, chk) {
        return parseFloat(attr(tr, ["", "win", "lost"][chk.value])) || 2.00; //赔率
    },
...
function setUserEvent(vs, sel) {
  ...
  allMatch = $id('input', 'lgList');
    ...
    function colChange() {// 切换欧赔与平均列
        var ini = this.id == 'bl_col' ? ['tab_op2', 'bold', 'ttd showrate', 7, 11] : ['bold', 'tab_op2', 'ttd showodds', 11, 7];
        op_col.className = ini[0];
        bl_col.className = ini[1];
        vstable.className = ini[2];
        if (IE) {//修复IE不渲染bug
            $id('tr', vstable)[3].cells[ini[3]].style.display = ''; //样式隐藏，它要清除内联样式
            $id('tr', vstable)[3].cells[ini[4]].style.display = 'block'; //强制刷新
        }
    };
    if (op_col) {
        on(op_col, 'click', colChange);
        on(bl_col, 'click', colChange);
    };
