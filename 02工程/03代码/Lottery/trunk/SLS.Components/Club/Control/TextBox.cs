        ...
        protected override void CreateChildControls()
        {
            ...
            if ((RequiredFieldType != null) && (RequiredFieldType != "") && (RequiredFieldType != "暂无校验"))
            {
                ...
                switch (RequiredFieldType)
                {
                    ...
                    case "身份证号码":
                        {        
                            RequiredFieldTypeREV.ValidationExpression = (this.ValidationExpression != null) ? this.ValidationExpression : "^\\d{15}$|^\\d{17}[XxYy0-9]$|^[A-Za-z][12]\\d{8}$|^[A-Za-z]{1,2}\\d{6}\\([Aa0-9]\\)$|^\\d{7}\\([0-9]\\)$|^\\d{7}[A-JZa-jz]$";
                            RequiredFieldTypeREV.ErrorMessage = "请输入正确的身份证号码！"; break;
                        }
                    case "网页地址":
                        {
